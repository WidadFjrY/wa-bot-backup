      ...
      for (let i = 0; i < res.links.length; i++) {
        ...
        const req = https.get(link.url, (result) => {
          ...
          fileOutput.on("finish", () => {
            ...
            let fileSize = fileSizeInMegabytes.toFixed(2);
                  ...
                  setTimeout(() => {
                    if (i === res.links.length - 1) {
                      const content =
                        ...
                        "\n\n*Caption*\n " +
                        caption +
                        "\n\n*Notes*\nFile yang lebih dari 15MB akan dikirim melalui Document";
                      client.sendMessage(message.from, content);
                      sendingMessage(pushname, url);

                      db.query(
                        "UPDATE tools_count SET total_images = total_images + ?, total_videos = total_videos + ?, instagram = instagram + 1 WHERE number = ?",
                        [imgCount, vidCount, newNumber],
                        (error, result) => {
                          if (error) {
                            console.error(
